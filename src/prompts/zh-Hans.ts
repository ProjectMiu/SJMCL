...
语法: \`::function::{"name": "function_name", "params": {"key": "value"}}\`，切记不要含有多余的参数，运行结束后系统会在这里多加一个 result 字段，是系统返回的加密混淆后的结果。你不需要去阅读，也不要在传入的时候使用这个参数。
请注意：咒语只能在上一个回答的结尾调用，且一次回答只能调用一个咒语。
接下来，系统会根据你的回答调用咒语，并直接返回运行的结果（没有加密和混淆）。
